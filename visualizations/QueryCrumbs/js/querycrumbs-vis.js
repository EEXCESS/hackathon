...
function display_querycrumbs(domElem) {
    ...
    $(document).on("click", "#eexcess_stop_btn", function() {
        ...
        var succ = function(queries) {
            evaluationObject[evaluationUserID][taskID] = {
        ...
        var err = function(err_params) {
            evaluationObject[evaluationUserID][taskID] = {
                completionTime: inSeconds
            };
            localStorage.setItem("evaluation", JSON.stringify(evaluationObject));
            display_status("Task stopped (wo queries)")
        };
        EEXCESS.storage.getQHist(taskStart, succ, err);
    })
                ...
                if (QueryCrumbsConfiguration.skillLevel != "BEGINNER") {
                    ...
                    for (var n in iDocs) {
                        var docRects = rootGroup.selectAll("g.queryNode")
                                .filter(function(d, i) {
                            return (d.timestamp == visualData.visualDataNodes[n].timestamp);
                        })
                                .select("g").selectAll("rect.docNode")
                                .filter(function(d, i) {
                            return (iDocs[n].indexOf(i) != -1);
                        });
    ...
    var RENDERING = {
        ...
        redraw: function(visualData) {
            ...
            if (QueryCrumbsConfiguration.nodeForm == "CIRCLE") {
                ...
                var queryNodesSel = crumbsUpd.selectAll("g.queryNode").data(function(d) {
                    return d.visualDataNodes;
                }, function(d) {
                    return d.timestamp;
                });
                ...
                nodeEnter.append("circle").attr("class", "queryCircleBg").attr({
                    ...
                    r: function(d, i) {
                        return ((i == currentIdx) && (dataTotal > 0)) ? QueryCrumbsConfiguration.dimensions.circle_bg_r + 1 : QueryCrumbsConfiguration.dimensions.circle_bg_r
                    }
                }).classed("queryRectBg-selected", function(d, i) {
                    return ((i == currentIdx) && (dataTotal > 0));
                });
                ...
                nodeEnter.transition().attr("transform", function(d, i) {
                    return ("translate(" + (d.x_pos + 2) + ", 0)");
                });


                var arc = d3.svg.arc().outerRadius(QueryCrumbsConfiguration.dimensions.circle_r);

                queryNodesSel.select("g").attr("transform", function(d, i) {
                    return ("translate(25, 25)");
                });

                var queryDocRects = queryNodesSel.select("g").selectAll("path.docNode").data(function(d) {
                    return d.results;
                });
                ...
                nodeEnter.append("g");
                queryNodesSel.exit().transition().style("opacity", 0).attr("transform", function(d, i) {
                    if (fWait_BackNaviResults) {
                ...
                queryNodesSel.select("rect.queryRect").attr("x", function(d) {
                    return d.x_pos;
                })
                        .attr("y", function(d) {
                    return d.y_pos;
                ...
                var queryDocRects = queryNodesSel.select("g").selectAll("rect.docNode").data(function(d) {
                    return d.results;
                });
...
//                .style("opacity", function(d) { return d.simTerms;});
        ...
        transitionTo: function(visualData, currIdx) {
            var nodesToFadeOut = svgContainer.selectAll("g.queryNode").filter(function(d, i) {
                return (i > currIdx);
            });
